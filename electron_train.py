...
from lib.networks.electron_training import train, eval
from lib.networks.utils import cnt_params
from datetime import datetime
from prepare_electron_data import get_data


def define_options_parser():
    parser = argparse.ArgumentParser(description='Model training script. Provide a suitable config.')
    parser.add_argument('--config', type=str, default='./configs/electron_model.yaml', help='Path to config file in YAML format.')
    parser.add_argument('--modelname', type=str, default = 'electron_model', help='Model name for saving checkpoints.')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    """
    dataset = get_data()
    train_dataset = ElectronData(dataset)
    eval_dataset = ElectronData(dataset) 
