...
def one_hot_cloud(data, num_labels, position):
    ...
    encoded_tensor =  torch.zeros(data.shape[0],5,data.shape[2])
...
class Flow_Mixture_Model(Local_Cond_RNVP_MC_Global_RNVP_VAE):
    ...
    def decode(self, p_input, g_sample, cloud_labels, n_sampled_points, labeled_samples=False, warmup=False):
        '''
        ...
        for i in range(self.n_components):
            ...
            one_decoder = self.one_flow_decode(p_input, g_sample, cloud_labels, self.pc_decoder[i], sampled_cloud_size[i])
        ...
        if labeled_samples:     #when for evaluation
            ...
            samples = torch.zeros_like((p_input_labeled))
            labels = torch.zeros(p_input_labeled.size(0), p_input_labeled.size(2))
            for t in range(self.n_components):
