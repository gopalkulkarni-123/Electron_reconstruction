...
def one_hot_condition(data, num_labels, position):
    data = data.cpu()
    position = position.cpu()
    encoded_tensor =  torch.zeros(5,3,128)
...
class Local_Cond_RNVP_MC_Global_RNVP_VAE(nn.Module):
    ...
    def one_flow_decode(self, p_input, g_sample, label, pc_decoder, n_sampled_points):
        '''
        ...
        if self.mode == 'training':
            #train decoder flow
            p_input_labeled = one_hot_cloud(p_input, 2, label)
            g_sample_labeled = one_hot_condition(g_sample, 2, label)
            ...
            p_prior_samples_labeled = one_hot_encode(output['p_prior_samples'][0], 2, label)
            buf = pc_decoder(p_prior_samples_labeled, g_sample_labeled, mode='direct')
    ...
    '''
    def forward(self, g_input, p_input, label, images=None, n_sampled_points=None, labeled_samples=False, warmup=False):
        '''
        main function
        ...
        if labeled_samples:
            samples, labels, mixture_weights_logits = self.decode(p_input, g_sample, label, sampled_cloud_size, labeled_samples, warmup)
            return output_encoder, samples, labels, mixture_weights_logits
        else:
            #print("decoding")
            output_decoder, mixture_weights_logits = self.decode(p_input, g_sample, label, sampled_cloud_size, labeled_samples, warmup)
