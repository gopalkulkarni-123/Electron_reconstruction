...
#for loop starts
    for i, batch in enumerate(iterator):
        ...
        loss, pnll, gnll, gent = loss_func(output_prior, cloud_labels, output_decoder, mixture_weights_logits)
        with torch.no_grad():
            if torch.isnan(loss):
...
def eval(iterator, model, loss_func, optimizer, epoch, iter, warmup, min_loss, eval_writer, **kwargs):
    ...
    for i, batch in enumerate(iterator):
        ...
        g_clouds = batch['cloud'].cuda(non_blocking=True)
        p_clouds = batch['eval_cloud'].cuda(non_blocking=True)
        cloud_labels = batch['label']
        output_prior, output_decoder, mixture_weights_logits = model(g_clouds, p_clouds, cloud_labels, images=None, n_sampled_points=None, labeled_samples=False, warmup=warmup)
    ...
    if kwargs.get('logging_img') and epoch % kwargs.get('logging_img_frequency') == 0 and kwargs.get('logging'):
        ...
        if kwargs.get('distributed'):
            ...
            all_samples, all_gts, all_labels = reconstruct(iterator, model, cloud_labels, max_batches=1, warmup=False, **kwargs)
...
def reconstruct(test_dataloader, model, cloud_labels, max_batches=np.infty, warmup=False, **config):
        ...
        with torch.no_grad():
            for j in range(g_input.shape[0]):
                if train_mode == 'p_rnvp_mc_g_rnvp_vae':
                    output_prior, samples, labels, log_weights = model(g_input[j].unsqueeze(0), p_input[j].unsqueeze(0), cloud_labels,
