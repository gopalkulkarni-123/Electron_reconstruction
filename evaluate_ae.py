...
from lib.datasets.datasets import ElectronData
...
from prepare_electron_data_v2 import electron_dataset


def define_options_parser():
    parser = argparse.ArgumentParser(description='Model training script. Provide a suitable config.')
    parser.add_argument('--experiment_path', type=str, default = r"D:\Masters\Univerities\TU Dresden\Post_Admit\Studies\4th Sem\RP\Electron_reconstruction\results\airplane_gen_model", help='Path to experiment.')
    parser.add_argument('--modelname', type=str, default = 'airplane_gen_model_250', help='Model name (without ending).')
    parser.add_argument('--part', default = 'test', help='Part of dataset (train / val / test).')
    parser.add_argument('--cloud_size', default = 50000, type=int, help='Number of input points.')
    parser.add_argument('--sampled_cloud_size', default = 50000, type=int, help='Number of sampled points.')
    parser.add_argument('--mode', type=str, default = 'generating', help='Prediction mode (training / evaluating / generating / predicting).')
...
if config['train_mode'] == 'p_rnvp_mc_g_rnvp_vae':
    ...
    """
    eval_dataset = ElectronData(electron_dataset)
